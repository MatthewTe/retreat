...
def check_feeds_for_changes(feeds: List[str], db_path: str) -> list[CoreRSSFeeds]:
    ...
    if len(feeds) == 0:
        logger.info(f"No feeds provided for article ingestion - querying all feeds for tracking changes")
        all_feed_raw_rows  = conn.execute("SELECT url FROM rss_feeds").fetchall()
        feeds = [feed[0] for feed in all_feed_raw_rows]
...
def check_articles_for_changes(feeds: List[CoreRSSFeeds], db_path: str) -> Tuple[List[Dict[str, Any]], Optional[Exception]]:
    ...
    if len(feeds) == 0:
        logger.info(f"No feeds provided for article ingestion - querying all feeds for tracking changes")
        all_feed_raw_rows = conn.execute("SELECT feedContent FROM rss_feeds").fetchall()
        feeds: CoreRSSFeeds = [feedparser.parse(feed[0]) for feed in all_feed_raw_rows]
    ...
    for feed_title, articles in feed_new_articles.items():
        ...
        with sync_playwright() as p:
            ...
            for item in articles:
                content = extract_content_via_playwright(page, item["link"])
                timestamp = reconcile_feed_timestamp(item)
